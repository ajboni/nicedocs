...
var stripBomString = function stripBomString(str) {
...
getCjsExportFromNamespace(_nodeResolve_empty$1);

var MarkdownIt = require('markdown-it');

var emoji = require('markdown-it-emoji');

var mark$1 = require('markdown-it-mark');

var anchor = require('markdown-it-anchor');

function LoadDocs() {
  var lang = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "all";
  // Set the store value
  var loadedDocs = {};
  ...
  availableLanguages.forEach(function (lang) {
    var path = './docs/' + lang.id;
    loadedDocs[lang.id] = loadDocsRecursively(path, lang);
  });
  return loadedDocs; // if (!fs.existsSync(path)) {
  ...
  // 		console.log(`Language folder: ${path} not found. Using default language: ${config.defaultLanguage}`)
  // 	}
  // 	else {
...
function loadDocsRecursively(path, lang) {
  var level = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
  var loadedDocs = fs.readdirSync(path);
  var docs = [];
  loadedDocs.forEach(function (docFile) {
    var obj = {};
    var isFolder = fs.lstatSync(path + "/" + docFile).isDirectory();
    obj.title = formatTitle(docFile);
    obj.lang = lang;

    if (isFolder) {
      obj.type = "folder";
      obj.children = loadDocsRecursively(path + "/" + docFile, lang, level + 1);
      ...
      obj.content = parseContent(fs.readFileSync(path + "/" + docFile).toString(), generateSlug(path + "/" + docFile)); // docsMap.set(obj.slug, obj)

      get_store_value(docsMap).set(obj.slug, obj);
    }
...
function parseContent(content, slug) {
  ...
  md.use(anchor, {
    permalink: true,
    permalinkSymbol: '🔗',
    permalinkHref: function permalinkHref(s) {
      return "".concat(slug, "#").concat(s);
    }
  });
...
function generateSlug(path) {
  var slug = path.replace("./docs/", "").replace(".md", "");
  var newSlug = [];
  var slugChunks = slug.split("/");

  for (var index = 0; index < slugChunks.length; index++) {
    var s = slugChunks[index];
...
var file = "src/routes/crawler/index.svelte";
...
function instance($$self, $$props, $$invalidate) {
  ...
  var _iterator = _createForOfIteratorHelper(get_store_value(docsMap).entries()),
      _step;
  ...
  Object.keys($$props).forEach(function (key) {
    if (!~writable_props.indexOf(key) && key.slice(0, 2) !== "$$") console.warn("<Crawler> was created with unknown prop '".concat(key, "'"));
  });
  var _$$props$$$slots = $$props.$$slots,
      $$slots = _$$props$$$slots === void 0 ? {} : _$$props$$$slots,
      $$scope = $$props.$$scope;
  validate_slots("Crawler", $$slots, []);
...
var Crawler = /*#__PURE__*/function (_SvelteComponentDev) {
  _inherits(Crawler, _SvelteComponentDev);

  var _super = _createSuper(Crawler);
